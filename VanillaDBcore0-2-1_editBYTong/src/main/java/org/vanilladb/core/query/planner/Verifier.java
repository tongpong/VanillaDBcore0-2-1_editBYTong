...
public class Verifier {
	public static String transqry(QueryData data, Transaction tx,String qry){
		...
		for (Schema s : schs){
			String [] read = s.toString().trim().split("\\s+|,|\\[|\\]");
		...
		//		System.out.println(read[i]);
				if(read[i].length()!=0){
					if(datatype==false){
						datatype=true;
						column_name=column_name+read[i]+',';
					}
